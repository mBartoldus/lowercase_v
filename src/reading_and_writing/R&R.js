...
export function _copy_at_index(output, input, index = 0) {
    let offset = output.length * index
    for (let i = 0; i < output.length; i++) output[i] = input[offset + i] || 0
    return output
}

export function _interpret_s(interpreter, ...args) {
...
export function _interpret_v(interpreter, ...args) {
...
export function _interpret_m(interpreter, ...args) {
...
export function _interpret_p(expected_domain, ...input) {
...
export const _reading_and_writing = {
    index: undefined,
    parent: undefined,
